...
export default async function ChangeFilters(ctx: SlackActionMiddlewareArgs<BlockStaticSelectAction> & AllMiddlewareArgs<StringIndexed>) {
    await ctx.ack();
    ...
    options.filters.publish_state = ctx.action.selected_option.value;
